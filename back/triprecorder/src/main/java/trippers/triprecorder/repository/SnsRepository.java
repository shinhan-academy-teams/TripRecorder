...
public interface SnsRepository extends JpaRepository<SnsVO, Long>{
	...
	// 특정 여행(sns)이 여기에 포함되고(팔로잉중인 사용자의 모든 TripVO) 공개범위가 0, 1 또는 내 게시글 (for. 메인화면 - 로그인 후)
	...
	List<SnsVO> findBySnsAndSnsScopeInOrderBySnsNoDesc(TripVO trip, Integer[] scope);
	...
	// 특정 해시태그를 포함하고 (팔로우하는 사용자의 게시글 중 공개 범위가 팔로우 공개) or (내 게시글 중 전체 공개가 아님) or (전체 공개) (for. 해시태그 검색 - 로그인 후)
