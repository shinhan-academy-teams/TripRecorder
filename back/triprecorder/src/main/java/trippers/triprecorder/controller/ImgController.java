...
public class ImgController {
	...
	public ExpDto processReceipt(@RequestBody JSONObject imageObj) throws IOException, ParseException {
		...
		// 사용처 추출
		String storeInfo = ((String) ((JSONObject) ((JSONObject) ((JSONObject) result.get("storeInfo")).get("name"))
				.get("formatted")).get("value"));
		...
		int totalPrice = Integer
				.valueOf((String) ((JSONObject) ((JSONObject) ((JSONObject) result.get("totalPrice")).get("price"))
						.get("formatted")).get("value"));
		...
		// ExpDto에 담아서 리턴
		ExpDto expDto = ExpDto.builder().expPlace(storeInfo).expAddress(addresses).expMoney(Long.valueOf(totalPrice))
				.expTime(finalDateString).build();

		// AwsUtil.deleteBucketObjects(new String[]
		// {imageObj.get("imageKey").toString()});
		return expDto;
